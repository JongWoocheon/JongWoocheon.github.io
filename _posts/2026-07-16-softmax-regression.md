...
$$
<p align="center">
  <img src="{{ '/images/网络架构.png' | relative_url }}" alt="softmax 回归网络架构">
</p>
...
### 负对数似然
1. [^2]似然函数
模型输出 $P(y|x;\theta)$，表示输入 $x$ 属于类别 $y$ 的概率。
那么整个数据集的似然函数是：
$$
...
2. 取对数
$$
\log L(\theta)=\sum_{i=1}^{n}\log P(y^{(i)}|x^{(i)})
$$
3. 转换为最小化问题
...
[^1]: 交叉熵（Cross Entropy）是衡量两个概率分布之间差异的指标，常用于机器学习中评估模型预测结果与真实标签的接近程度，越小表示预测越准确。公式为 $l(\mathbf{y}, \hat{\mathbf{y}}) = -\sum_{k=1}^{K} y_k \log \hat{y}_k$ 。

[^2]: 似然函数是在已知观测数据的条件下，用参数表示数据出现可能性的函数。
